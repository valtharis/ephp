...
class Validation{
    ...
    */
        ...
        foreach( new ArrayIterator($this->validation_rules) as $var=>$opt)
        {
            ...
            switch($opt['type'])
            {
                ...
                case 'url':
                    ...
                    break;
                case 'date':
                    $this->validateDate($var, $opt['min'], $opt['max'], $opt['required'],$opt['label']);
                    break;
                case 'time':
                    $this->validateTime($var, $opt['min'], $opt['max'], $opt['required'],$opt['label']);
                    break;
                case 'numeric':
                    $this->validateNumeric($var, $opt['min'], $opt['max'], $opt['required'],$opt['label']);
                    if(!array_key_exists($var, $this->errors))
                    {
                ...
                case 'string':
                    $this->validateString($var, $opt['min'], $opt['max'], $opt['required'],$opt['label']);
                    if(!array_key_exists($var, $this->errors))
                    {
    ...
    {
        if($required==false && mb_strlen($this->source[$var],$this->encoding) == 0)
        {
            return true;
        }
     ...
     *
     */
    private function validateString($var, $min=0, $max=0, $required=false, $label = '')
    {
        if($required==false && mb_strlen($this->source[$var],$this->encoding) == 0)
        {
            return true;
        ...
        if(isset($this->source[$var]))
        {
            if(mb_strlen($this->source[$var],$this->encoding) < $min)
            {
                $this->errors[$var] = $label . ' jest zbyt krótki';
            }
            elseif(mb_strlen($this->source[$var],$this->encoding) > $max)
            {
                $this->errors[$var] = $label . ' jest zbyt długi';
            }
            elseif(!is_string($this->source[$var]))
            {
                $this->errors[$var] = $label . ' jest nie prawidłowy';
            }
        }
    }
    private function validateDate($var, $min=0, $max=0, $required=false, $label = ''){
        if($required==false && mb_strlen($this->source[$var],$this->encoding) == 0)
        {
            return true;
        }
        if(isset($this->source[$var])){
            ...
            if(!checkdate($date[1], $date[2], $date[0])){
                $this->errors[$var] = $label . ' podana data jest nieprawidłowa';
            }
        }
    }
    private function validateTime($var, $min=0, $max=0, $required=false, $label = ''){
        if($required==false && mb_strlen($this->source[$var],$this->encoding) == 0)
        {
            return true;
        }
        if(isset($this->source[$var])){
            ...
            preg_match("/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/", $this->source[$var], $time);
            if(count($time)>0){
                
            }else{
                $this->errors[$var] = $label . ' - nieprawidłowy format czasu [hh:mm]';
            }
        }
    }
    /**
     ...
     *
     */
    private function validateNumeric($var, $min=0, $max=0, $required=false, $label = '')
    {
        if($required==false && mb_strlen($this->source[$var],$this->encoding) == 0)
        {
            return true;
        ...
        {
            $this->errors[$var] = 'Pole \''.$label.'\' posiada nie prawidłową wartość';
        }
    }
